...
namespace FS.Utils.Common
{
    ...
    public abstract class Req
    {
        ...
        public static string GetTruePath()
        {
            ...
            if (url.Contains<char>('.')) { url = url.Substring(0, url.LastIndexOf("/") + 1); }
        ...
        public static T QS<T>(string parmsName, T defValue, Encoding encoding)
        {
            ...
            if (HttpContext.Current == null) { return defValue; }
            var requestType = HttpContext.Current.Request.GetType();
            var property = requestType.GetProperty("QueryStringBytes", BindingFlags.Instance | BindingFlags.IgnoreCase | BindingFlags.NonPublic);
            //var queryBytes = (byte[])property.GetValue(HttpContext.Current.Request, null);
            var queryBytes = (byte[])PropertyGetCacheManger.Cache(property, HttpContext.Current.Request);
            var parms = HttpUtility.UrlDecode(queryBytes, encoding);
            if (string.IsNullOrWhiteSpace(parms)) { return defValue; }

            parmsName += "=";
            foreach (var str in parms.Split('&')) { if (str.ToLower().StartsWith(parmsName)) { return ConvertHelper.ConvertType(str.Substring(parmsName.Length), defValue); } }
            return defValue;
        }
        ...
        {
            return HttpContext.Current == null ? defValue : ConvertHelper.ConvertType(HttpContext.Current.Request.Form[parmsName], defValue);
        }
        ...
        public static string GetIP(bool transparent = false)
        {
            var ip = "";
            var context = HttpContext.Current;
            if (context != null)
            {
                ...
                if (string.IsNullOrWhiteSpace(ip)) { ip = context.Request.ServerVariables["HTTP_VIA"]; }
                if (string.IsNullOrWhiteSpace(ip)) { ip = context.Request.ServerVariables["REMOTE_ADDR"]; }

                if (string.IsNullOrWhiteSpace(ip) || !IsType.IsIP(ip)) { ip = context.Request.UserHostAddress; }
                // 本地IP判断
                if (ip == "::1" && context.Request.Url.Host == "localhost") { return "127.0.0.1"; }
                if (string.IsNullOrWhiteSpace(ip) || !IsType.IsIP(ip)) { return "0.0.0.0"; }
                return ip;
            }
            return Net.GetIP();
        }
        ...
        {
            if (HttpContext.Current == null) { return false; }
            for (var i = 0; i < HttpContext.Current.Request.Files.Count; i++) { if (HttpContext.Current.Request.Files[i].FileName != "") { return true; } }
            return false;
        }
        ...
        {
            if (HttpContext.Current != null && HttpContext.Current.Request.Files.Count > 0) { HttpContext.Current.Request.Files[0].SaveAs(path); }
        }
        ...
        public static void GoToUrl(string url = "")
        {
            ...
            if (url.StartsWith("?")) { url = GetPageName() + url; }
        ...
        public static TEntity Fill<TEntity>(NameValueCollection request, out Dictionary<string, List<string>> dicError, string prefix = "hl") where TEntity : class, new()
        {
            ...
            foreach (var kic in EntityPhysicsMapCacheManger.Cache(typeof(TEntity)).MapList)
            {
                ...
                else { PropertySetCacheManger.Cache(kic.Key, t, obj); }
            }
